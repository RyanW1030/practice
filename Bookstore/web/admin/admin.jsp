<%--
  ...
  2. [Request 属性]
     - 键名: "bookList" (ArrayList<Book>) : 书籍列表。
     - 键名: "userList" (ArrayList<UserInfo>) : 用户列表。
     - 键名: "orderList" (ArrayList<UserOrder>) : 订单列表。
     - 键名: "categoryList" (ArrayList<Category>) : 分类列表。
     - 键名: "add"/"update"/"delete"/"erase"/"recycleBooks"/"shipOrder" (Integer) : 结果状态码。
     - 键名: "recycleList" (String "yes") : 标记当前是否处于回收站模式。
     - 键名: "tab" (String) : 可选，用于后端强制指定 Tab。

  3. [Request 参数]
     - tab: "books" | "users" | "orders"
     - recycleList: "yes" (回收站模式)
     - categoryId, keyword: 搜索参数

  4. [交互接口]
     - 订单发货: action=shipOrder&order_id=XXX&tab=orders
     - 书籍操作: action=deleteBooks/eraseBooks...
  =================================================================================
--%>
...
<%
    ...
    // 获取当前激活的 Tab
    // 逻辑升级：优先检查 Attribute (后端强制跳转)，其次检查 Parameter (前端链接)
    ...
    Integer shipResult = (Integer) request.getAttribute("shipOrder"); // [新增] 发货结果
...
<head>
    ...
    <style>
        ...
        /* [核心修复] 强制不换行：解决状态和按钮上下割裂问题 */
        .text-nowrap {
            white-space: nowrap !important;
            min-width: 80px; /* 最小宽度保障 */
        }
    </style>
</head>
<body>
...
<div class="admin-layout">
    ...
    <aside class="admin-sidebar">
        <div class="admin-logo">
            <i class="fas fa-user-shield" style="margin-right: 10px;"></i> 管理后台
        </div>

        <nav class="sidebar-menu">
            <a href="<%=contextPath%>/adminServlet?action=List&tab=books"
               class="sidebar-item <%= "books".equals(currentTab) ? "active" : "" %>">
                <i class="fas fa-book"></i> 书籍管理
            </a>
            <a href="<%=contextPath%>/adminServlet?action=List&tab=orders"
               class="sidebar-item <%= "orders".equals(currentTab) ? "active" : "" %>">
                <i class="fas fa-clipboard-list"></i> 订单管理
            </a>
            <a href="<%=contextPath%>/adminServlet?action=List&tab=users"
               class="sidebar-item <%= "users".equals(currentTab) ? "active" : "" %>">
        ...
        <div class="admin-user-info">
            ...
            <a href="<%=contextPath%>/userServlet?action=logout" title="退出" style="margin-left: auto; color: #e74c3c;">
    ...
    <main class="admin-main">
        <header class="admin-header">
            ...
            <div>
                <a href="<%=contextPath%>/index.jsp" target="_blank" class="btn-sm btn-edit">
                    <i class="fas fa-external-link-alt"></i> 前往商城首页
                </a>
            </div>
        </header>
        ...
        <div class="admin-content">
            ...
            <div class="recycle-notice">
                ...
                <strong>回收站模式：</strong> 您正在查看已下架的书籍。彻底删除后将无法恢复。
            </div>
            <% } %>
            ...
            <div class="admin-card">
                <div class="admin-toolbar">
                    ...
                    <div style="display:flex; gap:10px; align-items:center;">
                        ...
                        <% if (isRecycleMode) { %>
                        <button class="btn-toolbar-action btn-action-restore" onclick="batchRecycleBooks()">
                            <i class="fas fa-trash-restore-alt"></i> 批量还原
                        </button>
                        <button class="btn-toolbar-action btn-action-dark" onclick="batchEraseBooks()">
                            <i class="fas fa-times-circle"></i> 彻底删除
                        </button>
                        <% } else { %>
                        <button class="btn-toolbar-action btn-action-danger" onclick="batchDeleteBooks()">
                            <i class="fas fa-trash-alt"></i> 批量下架
                        </button>
                        <button class="btn-toolbar-action btn-action-primary" onclick="openAddBookModal()">
                            <i class="fas fa-plus"></i> 新增书籍
                        </button>
                        <% } %>
                    </div>
                        ...
                        <form action="<%=contextPath%>/adminServlet" method="get" style="display:flex; gap:8px;">
                            ...
                            <select name="categoryId" class="admin-search-select" onchange="this.form.submit()">
                                ...
                                <% for(Category c : categoryList) {
                                    String selected = String.valueOf(c.getId()).equals(paramCategoryId) ? "selected" : "";
                                %>
                            ...
                            <input type="text" name="keyword" class="admin-search-input"
                                   placeholder="搜索书名/作者..." value="<%= paramKeyword %>">

                            <button type="submit" class="btn-search-icon" title="搜索">
                                <i class="fas fa-search"></i>
                            </button>
                        </form>
                        <a href="<%=contextPath%>/adminServlet?action=List&tab=books&recycleList=yes"
                           class="btn-toggle-mode" title="查看回收站">
                            <i class="fas fa-recycle"></i> 回收站
                        </a>
                        <% } else { %>
                        <a href="<%=contextPath%>/adminServlet?action=List&tab=books"
                           class="btn-toggle-mode" style="background:#eef6ff; color:var(--primary-color); border-color:#b3d7ff;">
                ...
                <table class="admin-table">
                    ...
                    <tr>
                        <th width="5%" class="text-center">
                            <input type="checkbox" id="checkAllBooks" onclick="toggleAllBooks(this)" class="book-check">
                        </th>
                    ...
                    <% for(Book b : bookList) {
                        ...
                        for(Category c : categoryList){
                            if(c.getId() == b.getCategory_id()) { categoryName = c.getName(); break; }
                        }
                    %>
                    <tr>
                        <td class="text-center">
                            <input type="checkbox" class="book-check" value="<%= b.getId() %>">
                        </td>
                        <td class="text-center"><%= b.getId() %></td>
                        <td class="text-center">
                            <img src="<%=contextPath%><%= b.getImg_path() %>" style="height: 40px; width: auto; border-radius: 4px; display:inline-block;">
                        </td>
                        <td class="text-left" title="<%= b.getName() %>">
                        ...
                        <td class="text-center">
                            <span class="badge badge-gray" style="white-space: nowrap; display: inline-block;"><%= categoryName %></span>
                        </td>
                        <td class="text-center" style="color: var(--accent-color); font-weight: bold;">￥<%= b.getPrice() %></td>
                        <td class="text-center">
                            <% if(b.getStock() < 0) { %>
                            <span style="color: #999;">已删除</span>
                            <% } else if(b.getStock() < 10) { %>
                            <span style="color: red; font-weight: bold;"><%= b.getStock() %></span>
                            <% } else { %>
                            <%= b.getStock() %>
                            <% } %>
                        </td>
                        ...
                        <td class="text-center text-nowrap">
                            ...
                            <div class="action-btn-group" style="justify-content: center;">
                                <% if (isRecycleMode) { %>
                                <button class="btn-sm btn-restore-sm"
                                        onclick="singleRecycleBook(<%= b.getId() %>, '<%= b.getName() %>')" title="还原">
                                    <i class="fas fa-trash-restore"></i>
                                </button>
                                <button class="btn-sm btn-dark-sm"
                                        onclick="singleEraseBook(<%= b.getId() %>, '<%= b.getName() %>')" title="彻底删除">
                                    <i class="fas fa-times"></i>
                                </button>
                                <% } else { %>
                                <button class="btn-sm btn-edit" onclick="openEditBookModal(<%= b.getId() %>)" title="编辑">
                                    <i class="fas fa-edit"></i>
                                </button>
                                <button class="btn-sm btn-danger"
                                        onclick="deleteSingleBook(<%= b.getId() %>, '<%= b.getName() %>')" title="下架">
            ...
            <div class="admin-card">
                <div class="admin-toolbar">
                    <span>共 <%= orderList.size() %> 笔订单</span>
                </div>
                <table class="admin-table">
                    <thead>
                    <tr>
                        <%-- [列宽调整] 增加状态和操作列的占比，确保不拥挤 --%>
                    ...
                    <% for(UserOrder o : orderList) {
                        ...
                        if(s == 0) {
                            statusLabel = "未付款"; statusClass = "badge-red"; // 警示色
                        } else if(s == 1) {
                            statusLabel = "待发货"; statusClass = "badge-blue"; // 待处理
                        } else if(s == 2) {
                            statusLabel = "已发货"; statusClass = "badge-orange"; // 运输中
                        } else if(s == 3) {
                            statusLabel = "已收货"; statusClass = "badge-green"; // 完成
                        } else {
                            statusLabel = "未知"; statusClass = "badge-gray";
                        }
                    %>
                    <tr>
                        <td class="text-left">
                            <span class="font-monospace"><%= o.getOrder_id() %></span>
                        </td>
                        <td class="text-center" style="font-size: 13px;">
                            <%= (o.getCreate_time() != null) ? sdf.format(o.getCreate_time()) : "--" %>
                        </td>
                        <td class="text-center"><%= o.getUser_id() %></td>
                        <td class="text-right" style="color: var(--accent-color); font-weight: bold; padding-right: 20px;">
                            ￥<%= o.getTotal_money() %>
                        </td>
                        ...
                        <td class="text-center text-nowrap">
                            <% if(s == 1) { %>
                            <%-- [参数修复] 添加 &tab=orders 参数，确保发货后停留在订单管理页 --%>
                            <a href="<%=contextPath%>/adminServlet?action=shipOrder&order_id=<%= o.getOrder_id() %>&tab=orders"
                               class="btn-sm btn-success">
                                <i class="fas fa-shipping-fast"></i> 立即发货
                            </a>
                            <% } else { %>
            ...
            <div class="admin-card">
                <div class="admin-toolbar">
                    <span>共 <%= userList.size() %> 位注册用户</span>
                </div>
                <table class="admin-table">
                    <thead>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td class="text-center">￥<%= u.getBalance() %></td>
                        <td class="text-center">
                            <div class="action-btn-group" style="justify-content: center;">
                                <a href="<%=contextPath%>/adminServlet?action=resetPwd&id=<%= u.getId() %>"
                                   class="btn-sm btn-edit"
                                   onclick="return confirm('确定要重置该用户密码为 12345678 吗？')">
                                    <i class="fas fa-key"></i> 重置密码
                                </a>
                                <a href="<%=contextPath%>/adminServlet?action=deleteUser&id=<%= u.getId() %>"
                                   class="btn-sm btn-danger"
                                   onclick="return confirm('警告：删除用户将级联删除其所有订单！确定吗？')">
...
<div class="modal-overlay" id="bookModal">
    <div class="modal-box" style="width: 700px;">
        <div class="modal-header">
            <h3 class="modal-title" id="modalTitle">书籍信息</h3>
        </div>
        ...
        <form id="bookForm" action="" method="post" enctype="multipart/form-data">
            ...
            <div class="admin-form-row">
                <div class="form-group" style="flex: 2;">
                    <label>书籍名称</label>
                    <input type="text" name="name" id="bookName" class="form-control" required placeholder="输入完整的书名">
                </div>
            ...
            <div class="admin-form-row">
                ...
                <div class="form-group">
                    <label>出版社</label>
                    <input type="text" name="publisher" id="bookPublisher" class="form-control" placeholder="例如：机械工业出版社">
                </div>
            </div>
            ...
            <div class="admin-form-row">
                <div class="form-group">
                    <label>ISBN 版号</label>
                    <input type="text" name="isbn" id="bookIsbn" class="form-control" placeholder="978-7-xxx-xxxx-x">
                    <small class="input-helper">请输入13位标准ISBN号</small>
                </div>
                <div class="form-group">
                    <label>价格 (￥)</label>
                    <input type="number" step="0.01" name="price" id="bookPrice" class="form-control" required>
                </div>
                <div class="form-group">
                    <label>库存数量</label>
                    <input type="number" name="stock" id="bookStock" class="form-control" required>
                </div>
            </div>

            <div class="form-group">
                <label>封面图片 (修改/上传)</label>
                <div class="file-upload-wrapper">
                    <input type="file" name="cover_img" id="fileInput" accept="image/*" onchange="previewImage(this)">
                    <div class="upload-placeholder">
                        <i class="fas fa-cloud-upload-alt"></i>
                        <span>点击上传图片 (支持 jpg, png, avif)</span>
                    </div>
                </div>
                <div class="img-preview-box" id="previewBox">
                    <img id="previewImg" src="" alt="封面预览">
                </div>
            </div>

            <div class="form-group">
                <label>书籍简介</label>
                <textarea name="intro" id="bookIntro" class="form-textarea" placeholder="请输入详细的书籍介绍内容..."></textarea>
            </div>
...
<script>
    ...
    // --- 结果反馈逻辑 ---
    <% if (addResult != null) { %>
    showToast(<%= addResult > 0 %> ? "添加图书成功！" : "添加图书失败，请重试。");
    <% } %>

    <% if (updateResult != null) { %>
    showToast(<%= updateResult > 0 %> ? "修改图书成功！" : "修改图书失败，请重试。");
    <% } %>

    <% if (deleteResult != null) { %>
    showToast(<%= deleteResult > 0 %> ? "成功移入回收站！" : "删除失败，请重试。");
    <% } %>

    <% if (eraseResult != null) { %>
    showToast(<%= eraseResult > 0 %> ? "彻底删除成功！" : "彻底删除失败。");
    <% } %>

    <% if (recycleResult != null) { %>
    showToast(<%= recycleResult > 0 %> ? "书籍还原成功！" : "还原失败。");
    <% } %>

    <% if (shipResult != null) { %>
    showToast(<%= shipResult > 0 %> ? "发货成功！" : "发货失败，请重试。");
    <% } %>
    ...
    function getSelectedIds() {
        const checkboxes = document.querySelectorAll('.book-check:checked');
    ...
    function singleEraseBook(id, name) {
        if (!confirm("警告：彻底删除《" + name + "》后将无法恢复！\n确定继续吗？")) return;
